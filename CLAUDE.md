...
| `URGE_TOO_FREQUENT` | 409 | 催办防重复窗口（默认可配置 24 小时，D1／13.9.5） | 二次确认弹窗，带 `force` 重试 |
...
### 三色灯（需求文档 13.4.1a · V-9 现行口径，设计规范 2.5）

| 灯 | 语义 | 色值 | 浅底 | 图标 | 必现文案 |
|---|---|---|---|---|---|
| 蓝灯 | 正常运行（健康态） | `#0EA5E9` | `#E0F2FE` | 对号 | 「正常运行 · 剩余 N 天」 |
| 黄灯 | 需要关注 | `#F59E0B` | `#FEF3C7` | 三角 | 「需要关注 · 剩余 N 天」 |
| 红灯 | 已逾期或状态停滞 | `#EF4444` | `#FEE2E2` | 实心圆感叹号 | 「已逾期 · 逾期 N 天」或「状态停滞 · 停滞 N 天」 |
| 无灯 | 无预警（算不出灯） | `neutral-600` | — | 无图标 | — |
...
| **蓝灯即健康态（V-9）** | 预警区三张卡（蓝／黄／红），不再单独设「健康对象数」卡；红灯用文案区分逾期与停滞两种成因 |
