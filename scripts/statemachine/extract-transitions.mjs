...
//
// 5.13 是汇总清单，不是权威值域来源——权威值域是各自的转换表与第 8／9 章字段清单。
//
// 曾有两处笔误（需求业务验收状态写 4、课程开发子状态写 2），一度登记在这里做差异豁免。
// **需求 V1.3 已把这两处改对，因此豁免表清空。** 不保留已失效的豁免是有意的：留着的话，
// 万一以后有人把 5.13 改回错的数字，脚本会把它当「已知差异」静默放过，而不是报警。
